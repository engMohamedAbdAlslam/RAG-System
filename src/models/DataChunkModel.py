...
class DataChunkModel(BaseDataModel):
    ...
    async def insert_many_chunks(self, chunks: list, batch_size: int = 100):
        ...
        return len(chunks)
    async def delet_chunks_by_project_id(self,project_id:ObjectId):
    ...
    async def get_project_chunks(self,project_id : ObjectId, page:int=1,page_size:int =50):
        ...
        records=await self.collection.find({"chunk_project_id":project_id}).skip((page-1)*(page_size)).limit(page_size).to_list(length=None)
        return [ 
            DataChunk(**record)
